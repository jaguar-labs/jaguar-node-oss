...
When the wizard detects unused disks (bare devices: no partitions, no filesystem, unmounted), it offers to configure them interactively: for each detected disk you answer `ledger`, `accounts`, `snapshots` (comma-separated for sharing), `all`, or `skip` — every role must land on exactly one disk (duplicates and gaps re-prompt). Hybrid mappings work: e.g. ledger alone on one NVMe (`/mnt/solana_ledger`) with accounts+snapshots sharing another (`/mnt/solana_accounts_snapshots`). Afterwards you choose whether the playbook formats/mounts the disks, or you prepare them yourself (which also generates the `disk-setup-<name>.sh` commands for exactly the assigned disks).
