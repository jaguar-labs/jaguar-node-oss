...
When `jito.enabled` is true the agave build SHALL pass `--no-build-validator-bins` (CLI/operator tools only — the validator comes from the Jito build); when false it SHALL run a full build that includes `agave-validator`.

#### Scenario: Non-Jito host has a validator after provisioning
- **WHEN** the playbook runs with `jito.enabled: false`
