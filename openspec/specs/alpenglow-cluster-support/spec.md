# alpenglow-cluster-support Specification

## Purpose
TBD - created by syncing change `add-alpenglow-cluster`. Defines what an alpenglow deployment provides beyond the wizard: the dedicated start script with the cluster's flag set, the `build-alpenglow.sh` source-build script for arbitrary agave refs, and the stamping/warning of the cluster's volatile, restart-sensitive preset values.
