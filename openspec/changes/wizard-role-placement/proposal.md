...
- **Role-first placement flow** replacing the selection-first assignment *and* the classic `separate`/`single` layout prompts:
  ...
  3. **Accounts** — same ceremony as ledger, default `/mnt/solana_accounts/accounts`.
- **Arrow-key menus** (`↑`/`↓` + Enter) for every fixed-choice prompt in the flow, via a reusable `menu_select` helper reading tty escape sequences — with a **non-tty fallback** to numbered plain-text input so scripted e2e (and any piped use) keeps working.
...
- e2e matrix rewritten for the new prompt sequence (non-tty fallback path); sample regeneration expected to differ only if defaults shift (targets: byte-identical for the default separate-style answers).
