...
- [x] 6.1 Detect bonds via /sys/class/net/bonding_masters; display each bond with members and active member; propose active (else first) member as the interface default
- [x] 6.2 Reject bond-master input at the prompt (re-prompt listing the bond's members), mirroring the role's fail-fast assert
