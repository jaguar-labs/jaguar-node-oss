...
- **WHEN** the role prompts finish but `snapshots` is unassigned
- **THEN** the wizard reports the missing role and restarts the role prompts for the same selection rather than generating a broken playbook
