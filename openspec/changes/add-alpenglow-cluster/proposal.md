...
- **Third cluster option: `alpenglow`** in the wizard's cluster prompt, with presets from the reference node:
  ...
  - Agave version `4.2.0-beta.0` (tag verified upstream): `solana_version` becomes a per-cluster preset via a new template token — testnet/mainnet keep `4.1.0-beta.3` (byte-identical output), alpenglow gets `4.2.0-beta.0` so node-base's CLI build matches the cluster; `build-alpenglow.sh` defaults its ref to `v4.2.0-beta.0` (still overridable — the cluster moves)
